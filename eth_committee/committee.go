...
func NewCommittee(comID uint32, clientCnt int, _node *node.Node, config *core.CommitteeConfig) *EthCommittee {
	com := &EthCommittee{
		config:             config,
		multiSignData:      &MultiSignData{},
		Node:               _node,
...
func (com *EthCommittee) Start(nodeId uint32) {
...
func (com *EthCommittee) StartWorker() {
	com.worker.start()
}

func (com *EthCommittee) Close() {
	if com.worker != nil {
		com.worker.close()
	}
}

func (com *EthCommittee) SetInjectTXDone(cid uint32) {
	atomic.AddInt32(&com.injectNotDone, -1)
}

/* 交易注入完成且交易池空即可停止 */
func (com *EthCommittee) CanStopV1() bool {
	return com.CanStopV2() && com.txPool.Empty()
}

/* 交易注入完成即可停止 */
func (com *EthCommittee) CanStopV2() bool {
 ...
 */
func (com *EthCommittee) NewBlockGenerated(block *core.Block) {
	if com.to_reconfig {
		// 关闭worker
...
func (com *EthCommittee) TXpool() *TxPool {
	return com.txPool
}
...
func (com *EthCommittee) SetMessageHub(hub core.MessageHub) {
	com.messageHub = hub
}
 ...
 */
func (com *EthCommittee) AddTBs(tbblock *beaconChain.TBBlock) {
...
/* 向信标链发起交易，更新委员会地址列表
 */
func (com *EthCommittee) AdjustRecordedAddrs(addrs []common.Address, vrfs [][]byte, seedHeight uint64) {
...
func (com *EthCommittee) HandleClientSendtx(txs []*core.Transaction) {
 ...
 */
func (com *EthCommittee) send2Client(receipts map[uint64]*result.TXReceipt, txs []*core.Transaction) {
	// 分客户端
...
func (com *EthCommittee) getBlockHeight() *big.Int {
...
/* 从对应的分片获取账户状态和证明
 */
func (com *EthCommittee) getStatusFromShard(addrList []common.Address) *core.ShardSendState {
 ...
 * 将新区块的信标发送到信标链
 */
func (com *EthCommittee) SendTB(tb *core.SignedTB) {
	com.messageHub.Send(core.MsgTypeComAddTb2TBChain, com.Node.NodeInfo.NodeID, tb, nil)
}

func (com *EthCommittee) GetEthChainBlockHash(height uint64) (common.Hash, uint64) {
...
func (com *EthCommittee) GetCommitteeID() uint32 {
	return com.Node.NodeInfo.ComID
}

// 该方法仅在重组后同步交易池时使用
// 由于接收到旧leader的交易之前，新leader已经接收到client发送的交易，所以需要将旧leader交易插到队列最前
func (com *EthCommittee) SetPoolTx(poolTx *core.PoolTx) {
	if com.txPool == nil {
...
func (com *EthCommittee) HandleGetPoolTx(request *core.GetPoolTx) *core.PoolTx {
	com.txPool.lock.Lock()
	defer com.txPool.lock.Unlock()
	com.txPool.r_lock.Lock()
	defer com.txPool.r_lock.Unlock()
...
func (com *EthCommittee) SetOldTxPool() {
	if com.txPool == nil {
		return
	}
	com.txPool.lock.Lock()
...
func (com *EthCommittee) UpdateTbChainHeight(height uint64) {
